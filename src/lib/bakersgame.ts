...
export function newBakersGame(seed?: number): BakersGameState {
  const deck: Card[] = [];
  for (const suit of SUITS) {
    for (let rank = 1; rank <= 13; rank++) {
      deck.push({ id: `bg-${suit}-${rank}`, suit, rank, faceUp: true });
    }
  }
  const shuffled = shuffle(deck, seed ?? Date.now());
  // Same as FreeCell: cols 0-3 get 7 cards, cols 4-7 get 6 cards
...
export function cloneBakersGame(s: BakersGameState): BakersGameState {
  return {
    ...s,
...
export function getMovingBGCards(state: BakersGameState, src: BGSrc): Card[] {
...
export function isValidBGSequence(cards: Card[]): boolean {
...
function maxMoveSize(state: BakersGameState, destIsEmpty?: boolean): number {
...
export function canBGMove(state: BakersGameState, src: BGSrc, dest: BGDest): boolean {
  const cards = getMovingBGCards(state, src);
  ...
  if (!isValidBGSequence(cards)) return false;
...
export function tryBGMove(state: BakersGameState, src: BGSrc, dest: BGDest): BakersGameState | null {
  if (!canBGMove(state, src, dest)) return null;
  const cards = getMovingBGCards(state, src);
  const s = cloneBakersGame(state);
...
export function autoBGToFoundation(state: BakersGameState, src: BGSrc): BakersGameState | null {
  const cards = getMovingBGCards(state, src);
  if (cards.length !== 1) return null;
  for (let i = 0; i < 4; i++) {
    const r = tryBGMove(state, src, { kind: "foundation", pile: i });
    if (r) return r;
  }
  return null;
}
...
function bgRankLabel(rank: number): string {
  if (rank === 1) return "A";
  if (rank === 11) return "J";
  if (rank === 12) return "Q";
  if (rank === 13) return "K";
...
export interface BakersGameHint {
  src: BGSrc;
  description: string;
}

export function findBakersGameHint(state: BakersGameState): BakersGameHint | null {
  const singleSrcs: BGSrc[] = [];
  ...
  for (const src of singleSrcs) {
    for (let pile = 0; pile < 4; pile++) {
      if (canBGMove(state, src, { kind: "foundation", pile })) {
        const cards = getMovingBGCards(state, src);
        return { src, description: `Move ${bgRankLabel(cards[0].rank)} to foundation` };
      }
    }
  }
  ...
  for (const src of allSrcs) {
    for (let col = 0; col < 8; col++) {
      ...
      const dest: BGDest = { kind: "tableau", col };
      if (canBGMove(state, src, dest)) {
        const cards = getMovingBGCards(state, src);
        ...
        return {
          ...
          description:
            destPile.length === 0
              ? `Move ${bgRankLabel(cards[0].rank)} to empty column`
              : `Move ${bgRankLabel(cards[0].rank)} onto ${bgRankLabel(destPile[destPile.length - 1].rank)}`,
        };
      }
    }
  }
  ...
  for (const src of singleSrcs) {
    for (let cell = 0; cell < 4; cell++) {
      if (canBGMove(state, src, { kind: "freecell", cell })) {
        const cards = getMovingBGCards(state, src);
        return { src, description: `Park ${bgRankLabel(cards[0].rank)} in a free cell` };
      }
    }
  }
