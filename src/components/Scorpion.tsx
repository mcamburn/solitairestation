...
export function Scorpion() {
  const [state, setState] = useState<ScorpionState | null>(null);
  const [history, setHistory] = useState<ScorpionState[]>([]);
  ...
  useEffect(() => {
    ...
    setGameStats(recordWin("scorpion", elapsed, state.moves, dailyModeRef.current));
  ...
  useEffect(() => {
    if (state && state.moves > 0) saveGame("scorpion", state);
  }, [state]);
  ...
  const reset = (seed?: number) => {
    if (state && state.moves > 0 && !state.won && !statsRef.current) {
      setGameStats(recordLoss("scorpion", state.moves, dailyModeRef.current));
    }
  ...
  const commit = (next: ScorpionState | null) => {
  ...
  commitRef.current = (dr: DragInfo, zone: string) => {
    if (zone.startsWith("col-")) {
      ...
      commit(tryScorpionMove(game, dr.col, dr.index, destCol));
    }
  };
  ...
  const handleCardClick = (col: number, index: number) => {
    ...
    if (!pile || pile.length === 0) {
      ...
      if (sel) {
        commit(tryScorpionMove(game, sel.col, sel.index, col));
      }
      setSel(null);
      return;
    }
    ...
    if (sel) {
      ...
      if (commit(tryScorpionMove(game, sel.col, sel.index, col))) return;
    }
    setSel({ col, index });
  };
  ...
  const canDeal = game.stock.length > 0;
    ...
    >
      ...
      <div className="game-controls glass mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl px-4 py-2.5 text-xs">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button
            onClick={() => commit(dealScorpionStock(game))}
            disabled={!canDeal}
            ...
            title={canDeal ? "Deal 3 cards to first 3 columns" : "No more stock cards"}
          >
            Deal
      ...
      {isDragging && dragRef.current && (
        <ScorpionGhost dragInfo={dragRef.current} ghostPos={ghostPos!} fanUp={fanUp} skin={skin} face={face} />
      )}
    </div>
  );
}
